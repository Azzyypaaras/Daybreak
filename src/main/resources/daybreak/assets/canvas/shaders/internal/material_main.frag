...
vec3 hdr_filmicToneMap(in vec3 x) {
  ...
  vec3 block = vec3(0);
	 if(frx_isWorldTheNether()) {
      float bl = l2_clampScale(0.03125, 1.0, blockLight);
  	  ...
  	  block = hdr_gammaAdjust(mix(vec3(bl, bl*0.875, bl*0.75), vec3(1, 0.635, 0.360), bl));
    }
       ...
       float bl = l2_clampScale(0.03125, 1.0, blockLight);
        ...
        float bl = l2_clampScale(0.03125, 1.0, blockLight);
    	  bl *= bl * mix(hdr_blockMinStr, hdr_blockMaxStr, userBrightness);
    	  block = hdr_gammaAdjust(mix(vec3(bl, bl*0.875, bl*0.75), vec3(1, 0.843, 0.560), bl));
      }
...
//This bit controls overworld ambient light
vec3 l2_ambientColor(float time){
	vec3 ambientColor = hdr_gammaAdjust(vec3(1, 0.976, 0.878)) * hdr_sunStr * hdr_relAmbient;
...
//Just a multiplier on skyless dims
vec3 l2_skylessLightColor(){
	return hdr_gammaAdjust(vec3(0.875));
}

vec3 l2_dimensionColor(){
	if (frx_isWorldTheNether()) {
		return hdr_gammaAdjust(mix(vec3(0.615, 0.082, 0.309), gl_Fog.color.rgb, 0.7));
	}
	else {
...
//The meat and potatoes for nether-likes
vec3 l2_skylessLight(vec3 normal, float userBrightness){
	if(frx_worldHasSkylight()){
		return vec3(0);
	} else {
		float yalign = dot(normal, vec3(0.115, 0.537, 0.225)); // a bit towards z for more interesting effect
		yalign = frx_isSkyDarkened()?abs(yalign):max(0,yalign);
		return yalign * hdr_skylessStr * l2_skylessLightColor() * userBrightness / 3.0;
	}
}
...
void main() {
	...
	if(frx_isGui()){
		...
		if(frx_worldHasSkylight()){
			...
			userBrightness = smoothstep(0.0, 0.135, base);
		}
		...
		a.rgb *= hdr_finalMult;
		a.rgb = pow(hdr_filmicToneMap(a.rgb), vec3(1.0 / hdr_gamma));
	}
